 ...
 */
   ...
   * @param {ParseCallback} [parse] - Can be used to parse the response if a non-PyON token is found. ResponseType must be "none" for this to take place.
  */
  ...
  */
   ...
   * @param {string} type - Identifies the unique command type, e.g., [options] list.
   * @param {string} text - The serialized command which can be passed to the telnet fah interface.
   * @param {ParseCallback} parse - Custom parse function to parse the response if a non-PyON token is found.
  */
  ...
  */
  findResponseFromString (str, tokenizer = new Tokenizer()) {
    return this.findResponseFromTokens(tokenizer.tokenize(str))
  }
