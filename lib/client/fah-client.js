 ...
 * @public
*/
class FahClient extends events.EventEmitter {
  /**
   * Create a new client
   * @param {net.Socket} socket - A tcp socket connection
  */
  constructor (socket) {
    super()
    this.socket = socket
  ...
  */
  disconnect (force = false) {
    ...
    return events.once(this.socket, 'end')
  }
  ...
  */
  async takeResponse (cmd) {
    const tokens = []
    for await (const [token] of events.on(this, 'token')) {
